...
const _ = db.command
const LG = db.collection('UpDown').doc('LegalGroundsCafe')
...
function initChart(canvas, width, height, dpr) {  
  ...
  var weekdays= {    
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#d6b160', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        ...
        data: [4, 10, 45, 40, 30, 15, 10, 7, 0, 0, 0, 0, 0, 0, 0], },]}
...
Page({
    data: {
        ...
        timeTable:[{realTimeTable:'Mon: 8:00 - 15:00'},{realTimeTable:'Tue: 8:00 - 15:00'},{realTimeTable:'Wed: 8:00 - 15:00'},{realTimeTable:'Thu: 8:00 - 15:00'},{realTimeTable:'Fri: 8:00 - 15:00'},{realTimeTable:'Sat: closed'},{realTimeTable:'Sun: closed'}],
      },
...
// **************** 点赞功能所需onLoad **************//
   ...
   wx.cloud.database().collection('UpDown').doc('LegalGroundsCafe').get().then(res=>{
...
// **************** 评论功能所需onLoad **************//
   // 以下是CommentList函数
   wx.cloud.database().collection("comments").doc('LegalGroundsCafe').get()
   .then(res=>{
...
// ************** 评论功能所需onLoad结束*************//
 ...
 zan: function (item_id) {
  ...
  remark(e){
    ...
    wx.cloud.database().collection('comments').doc('LegalGroundsCafe')
    .update({
    data:{
  ...
  delete: function(e) {
    ...
    // 同步云端删除
    db.collection('comments').doc('LegalGroundsCafe').update({
      data: {
        commentList: list
      }
    })
  ...
  isYourComments: function (e) {
    ...
    wx.cloud.database().collection('comments').doc('LegalGroundsCafe').get().then(res=>{
      this.setData({
