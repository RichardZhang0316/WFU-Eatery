...
const _ = db.command
const YM = db.collection('UpDown').doc('Yamas')
...
function initChart(canvas, width, height, dpr) {  
  ...
  var weekdays= {    
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#d6b160', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        ...
        data: [0, 0, 0, 15, 40, 24, 10, 9, 7, 10, 15, 25, 20, 7, 3], },]}
  ...
  var weekend= {  
    ...
    series: [{name: 'Pit', type: 'bar', label: {normal: {show: false, position: 'inside', color: 'white'}},itemStyle: {borderRadius: [4, 4, 0, 0], color: '#d6b160', shadowColor: 'rgba(0, 0, 0, 0.5)', shadowBlur: 2},  //Series设置
        // 👇 数据录入处 ！！暂定以10为scale进行模拟, 数据源: Google
        data: [0, 0, 0, 20, 30, 20, 9, 6, 6, 9, 10, 15, 7, 3, 2], },]}
...
// **************** 评论功能所需onLoad **************//
     ...
     wx.cloud.database().collection('UpDown').doc('Yamas').get().then(res=>{
      console.log("RYM查询成功",res);
      this.setData({
        RYM: res.data.ItemList // 所有items全量信息
      })
      }).catch(err=>{
      ...
      // 初始页面加载CommentList
      wx.cloud.database().collection("comments").doc('Yamas').get()
      .then(res=>{
...
// ************** 评论功能所需onLoad结束*************//
  ...
  zan: function (item_id) {
    ...
    remark(e){
      ...
      wx.cloud.database().collection('comments').doc('Yamas')
      .update({
      data:{
    ...
    delete: function(e) {
      ...
      // 同步云端删除
      db.collection('comments').doc('Yamas').update({
        data: {
          commentList: list
        }
      })
    ...
    isYourComments: function (e) {
      ...
      wx.cloud.database().collection('comments').doc('Yamas').get().then(res=>{
        this.setData({
